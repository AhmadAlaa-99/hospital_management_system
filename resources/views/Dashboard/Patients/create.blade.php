    ...
    <div class="col">
        <div class="card hms-form-card">
            <div class="card-header">
                <h5 class="mb-0 hms-form-title">اضافة مريض جديد</h5>
                <p class="mb-0 text-muted tx-13">أدخل بيانات المريض وفق نظام المستشفى</p>
            </div>
            <div class="card-body">
                <form action="{{route('Patients.store')}}" method="post" autocomplete="off" class="hms-form">
                    @csrf
                    <div class="row">
                        <div class="col-3">
                        ...
                        <div class="col">
                            ...
                            <input class="form-control" name="Date_Birth" type="date" value="{{ old('Date_Birth') }}" required>
                        </div>
                            ...
                            <select class="form-control" name="insurance_id">
                                ...
                                @foreach($insurances as $insurance)
                                    <option value="{{ $insurance->id }}" {{ old('insurance_id') == $insurance->id ? 'selected' : '' }}>
                                        {{ $insurance->name }} (خصم {{ $insurance->discount_percentage }}%)
                                    </option>
                                @endforeach
                            </select>
                        </div>
