...
class InvoicesRepository implements InvoicesRepositoryInterface
{
    ...
    public function update($request, $id)
    {
        ...
        $patientName = optional($invoice->Patient)->name ?: 'مريض';
        if ($invoice->doctor_id) {
            NotificationService::notifyDoctor(
                (int) $invoice->doctor_id,
                'اكتملت نتيجة المختبر للمريض: ' . $patientName
            );
        }
        if ($invoice->patient_id) {
            NotificationService::notifyPatient(
                (int) $invoice->patient_id,
                'اكتملت نتيجة المختبر الخاصة بك ويمكنك مشاهدتها من لوحة التحكم.'
            );
        }
